...
TEST(probe_html_minimal_page) {
    GpgMetrics m = gpg_metrics("index.html",
        "<!DOCTYPE html>\n"
...
TEST(probe_html_no_structural_nodes) {
    GpgMetrics m = gpg_metrics("page.html",
...
TEST(probe_xml_element_nodes) {
    GpgMetrics m = gpg_metrics("config.xml",
        "<?xml version=\"1.0\"?>\n"
        "<config>\n"
...
TEST(probe_xml_multiple_elements) {
    GpgMetrics m = gpg_metrics("pom.xml",
        "<project>\n"
...
TEST(probe_xml_no_crash) {
    GpgMetrics m = gpg_metrics("empty.xml",
...
TEST(probe_css_module_only) {
    GpgMetrics m = gpg_metrics("styles.css",
        "body {\n"
        "  margin: 0;\n"
...
TEST(probe_scss_variable_node) {
    GpgMetrics m = gpg_metrics("theme.scss",
        "$primary: #3498db;\n"
        "$secondary: #2ecc71;\n"
        "\n"
        ".button {\n"
...
TEST(probe_markdown_section_node) {
    GpgMetrics m = gpg_metrics("README.md",
        "# Project Title\n"
        "\n"
        "Description text.\n"
        "\n"
...
TEST(probe_markdown_multi_heading) {
    GpgMetrics m = gpg_metrics("docs.md",
        "# Title\n"
        "## Setup\n"
        "### Install\n"
        "## Usage\n");
...
TEST(probe_markdown_no_class_variable) {
    GpgMetrics m = gpg_metrics("notes.md",
        "# Notes\n\nSome content.\n");
    ASSERT_TRUE(m.ok);
...
TEST(probe_rst_module_only) {
    GpgMetrics m = gpg_metrics("docs.rst",
        "Project Title\n"
        "=============\n"
        "\n"
        "Description text.\n"
...
TEST(probe_rst_no_crash) {
    GpgMetrics m = gpg_metrics("api.rst",
...
TEST(probe_yaml_variable_nodes) {
    GpgMetrics m = gpg_metrics("config.yaml",
        "name: myproject\n"
        "version: 1.0.0\n");
    ASSERT_TRUE(m.ok);
...
TEST(probe_yaml_three_keys) {
    GpgMetrics m = gpg_metrics("app.yaml",
        "host: localhost\n"
        "port: 8080\n"
        "debug: true\n");
    ASSERT_TRUE(m.ok);
...
TEST(probe_toml_table_and_key) {
    GpgMetrics m = gpg_metrics("Cargo.toml",
        "[package]\n"
        "name = \"mylib\"\n"
        "version = \"0.1.0\"\n");
    ASSERT_TRUE(m.ok);
...
TEST(probe_toml_multiple_tables) {
    GpgMetrics m = gpg_metrics("pyproject.toml",
        "[tool.poetry]\n"
        "name = \"demo\"\n"
        "\n"
...
TEST(probe_ini_section_and_key) {
    GpgMetrics m = gpg_metrics("app.ini",
        "[database]\n"
        "host = localhost\n"
        "port = 5432\n");
    ASSERT_TRUE(m.ok);
...
TEST(probe_ini_conf_extension) {
    GpgMetrics m = gpg_metrics("server.conf",
        "[server]\n"
        "listen = 0.0.0.0\n");
    ASSERT_TRUE(m.ok);
...
TEST(probe_csv_module_only) {
    GpgMetrics m = gpg_metrics("data.csv",
        "id,name,score\n"
        "1,Alice,95\n"
        "2,Bob,87\n"
        "3,Carol,92\n");
...
TEST(probe_sql_variable_node) {
    GpgMetrics m = gpg_metrics("schema.sql",
        "CREATE TABLE users (\n"
...
TEST(probe_sql_insert_select) {
    GpgMetrics m = gpg_metrics("queries.sql",
...
TEST(probe_soql_module_only) {
    GpgMetrics m = gpg_metrics("query.soql",
...
TEST(probe_sosl_module_only) {
    GpgMetrics m = gpg_metrics("search.sosl",
...
TEST(probe_dotenv_local_suffix) {
    GpgMetrics m = gpg_metrics(".env.local",
...
TEST(probe_gitignore_module_only) {
    GpgMetrics m = gpg_metrics(".gitignore",
        "*.o\n"
        "*.a\n"
        "build/\n"
        "dist/\n"
...
TEST(probe_gitattributes_module_only) {
    GpgMetrics m = gpg_metrics(".gitattributes",
        "*.c text eol=lf\n"
        "*.h text eol=lf\n"
        "*.png binary\n"
        "*.jpg binary\n");
...
TEST(probe_properties_module_only) {
    GpgMetrics m = gpg_metrics("app.properties",
...
TEST(probe_requirements_module_only) {
    GpgMetrics m = gpg_metrics("requirements.txt",
        "flask==2.3.0\n"
        "requests>=2.28.0\n"
        "sqlalchemy~=2.0\n");
    ASSERT_TRUE(m.ok);
...
TEST(probe_diff_module_only) {
    GpgMetrics m = gpg_metrics("changes.diff",
        "--- a/main.c\n"
        "+++ b/main.c\n"
        "@@ -1,5 +1,6 @@\n"
        " #include <stdio.h>\n"
...
TEST(probe_po_module_only) {
    GpgMetrics m = gpg_metrics("messages.po",
        "# Translation file\n"
        "msgid \"\"\n"
        "msgstr \"\"\n"
...
TEST(probe_regex_module_only) {
    GpgMetrics m = gpg_metrics("patterns.re",
        "(foo|bar)+\n"
        "^[a-zA-Z_][a-zA-Z0-9_]*$\n"
        "\\d{4}-\\d{2}-\\d{2}\n");
    ASSERT_TRUE(m.ok);
...
TEST(probe_sshconfig_module_only) {
    GpgMetrics m = gpg_metrics("ssh_config",
        "Host bastion\n"
        "  HostName 10.0.0.1\n"
        "  User ubuntu\n"
...
TEST(probe_bibtex_module_only) {
    GpgMetrics m = gpg_metrics("refs.bib",
        "@article{smith2023,\n"
        "  author = {Smith, John},\n"
...
TEST(probe_form_variable_node) {
    GpgMetrics m = gpg_metrics("calc.frm",
        "Symbol x, y, z;\n"
        "Local F = x^2 + y^2 + z^2;\n"
        "Print;\n"
        ".end\n");
...
TEST(probe_form_procedure) {
    GpgMetrics m = gpg_metrics("proc.frm",
        "#procedure Square(x)\n"
        "  id `x'^2 = `x'^2;\n"
        "#endprocedure\n"
        "\n"
...
TEST(probe_linkerscript_module_only) {
    GpgMetrics m = gpg_metrics("link.ld",
        "SECTIONS\n"
        "{\n"
        "  . = 0x10000;\n"
        "  .text : { *(.text) }\n"
...
TEST(probe_linkerscript_memory_sections) {
    GpgMetrics m = gpg_metrics("stm32.ld",
        "MEMORY\n"
        "{\n"
